...
workspace package (`@workspace/healers-inc`): other artifacts depend
...
- `docs/references/` — retained source evidence: the user's product UI
  screenshots (subjects and extraction notes in `docs/references/README.md`)
  and the real brand logo in `docs/references/logos/logo.svg` (sanitized; also
  copied to `public/logo.svg` for the preview header and Brand > Logo page).
  When rebuilding one of these screens, start from the matching capture.
...
`@workspace/healers-inc/components/...`. Never use a `@/` alias inside
...
for `@workspace/healers-inc` and renders mockups using this package's
theme and components.

## Consuming this package

Never copy token values, component source, hooks, or these docs into a consuming
artifact. Add `@workspace/healers-inc` as a `workspace:*` dependency,
run `pnpm install`, and import directly from this package.

Read only the guides required by the current task:

- Building or styling web UI: `artifacts/healers-inc/docs/consuming-web.md`
- Building or styling Expo UI: `artifacts/healers-inc/docs/consuming-expo.md`
- Replacing an existing or scaffolded web theme/component library:
  `artifacts/healers-inc/docs/migrating-web.md`
- Replacing existing or scaffolded Expo theme/hooks/components:
  `artifacts/healers-inc/docs/migrating-expo.md`
