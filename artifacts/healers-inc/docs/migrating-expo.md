# Migrating Expo UI to Healers Inc

Read `artifacts/healers-inc/docs/AGENTS.md` and
`artifacts/healers-inc/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/healers-inc/lib/native-theme` and
  `@workspace/healers-inc/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/healers-inc/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/healers-inc` and retains only
